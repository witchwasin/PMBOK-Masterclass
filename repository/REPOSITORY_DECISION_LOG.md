...
| 9 | 2026-07-31 | เก็บ `repository/PMBOK-Aligned-End-to-End-Project-Delivery-Playbook-V2.md` ไว้เพิ่มเติม — ประเมินว่าดีกว่า v1 และเป็น preferred source สำหรับ `field-guide/` ในอนาคต (v1 ยังเก็บไว้ ไม่ลบ) | v2 เพิ่มชั้น Execution Ownership (RACI ต่อกิจกรรม, Action Flow table ทุกช่วง A–H, Quick Role-to-Action Reference §3.1) ที่ v1 ไม่มี ทำให้ actionable กว่าสำหรับ field-manual use case | User (Owner) + Assistant comparison | เมื่อเริ่มเขียน `field-guide/` บทเนื้อหาแต่ละบทควรอ้าง v2 เป็นหลัก ใช้ v1 เป็น fallback/cross-check เท่านั้น |
| 10 | 2026-07-31 | คัดลอก v1 และ v2 ของ Playbook ไปไว้ที่ `references/` เพิ่มเติม (นอกเหนือจาก `repository/`) พร้อม status note กำกับว่าไม่ใช่ Canonical Source | User ต้องการให้ไฟล์อยู่ใน `references/` ด้วย — ทั้งสองชุดยังคงสถานะ Pending External Reference เหมือนเดิม ไม่กระทบสถานะ Canonical ของ `references/PMBOK-Overview.md` | User (Owner) | มีไฟล์ซ้ำ 2 ชุดใน `repository/` และ `references/` โดยตั้งใจ — หากแก้ไขในอนาคตต้อง sync ทั้งสองที่ หรือพิจารณาลดเหลือที่เดียว |
| 11 | 2026-07-31 | เขียน `field-guide/appendices/Appendix-E-SDLC-Role-Output-Matrix.md` เป็น Appendix แรกของเล่มที่มีเนื้อหาจริง (ก่อน Phase 1 Pilot Chapter) พร้อม PDF แนวนอน 1 หน้า ที่ `field-guide/pdf/Appendix-E-SDLC-Role-Output-Matrix.pdf` | สรุป PM ทำ vs ทีมทำ vs Output ต่อ Phase A–H ที่เกิดจากบทสนทนา ผู้ใช้ประเมินว่าคุณภาพดีพอใช้ได้เลย — reassigned Appendix E เดิม ("Problem→Chapter Index") เป็น Appendix F แทน | User (Owner) | Blueprint อัปเดตแล้วให้ตรงกับ numbering ใหม่ (E=Role/Output Matrix, F=Problem Index) — เมื่อเขียน Appendix อื่นต่อควรเช็ค numbering นี้ก่อน |
| 12 | 2026-07-31 | ยืนยันว่า "ebook v2" ที่ผู้ใช้เรียกในบทสนทนา คือ `field-guide/BOOK-BLUEPRINT.md` โปรเจกต์เดียวกัน (ไม่ใช่ V2 ของ `e-Book/` เดิม) — เพิ่ม alias ใน frontmatter และเปลี่ยน primary reference link เป็นสำเนาใน `references/` ตามที่ผู้ใช้ระบุ | ผู้ใช้ถามให้เขียนแผน "ebook v2" ใหม่ — เช็คก่อนแล้วพบว่าหมายถึงแผนเดิมที่มีอยู่แล้ว ไม่ใช่แผนใหม่ | User (Owner) + Assistant clarification | ยังไม่เริ่มเขียน Chapter ใดๆ ตามที่ผู้ใช้ขอ ("ทำเป็น md ไว้ก่อน แล้วค่อยมาทำวันหลัง") |
