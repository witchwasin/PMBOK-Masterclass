...
- **ควรรู้แล้ว:** L12 status and escalation; **คำศัพท์ใหม่:** risk, trigger, issue, change.
- **Novice trap:** บันทึกเหตุการณ์ที่เกิดแล้วเป็น risk และไม่เปิด issue/change control.
...
**[Teaching Scenario]** ERP Transformation มี data migration จาก 6 legacy systems, 80 key users, SI TechConsult และ go-live target ก่อน 1 ตุลาคม.
...
**[Teaching Scenario]** Hotel Booking Platform พึ่ง payment gateway, hotel partner inventory และ launch campaign เพื่อสร้าง direct booking outcome.
...
**[Enterprise Practice]** ความเสี่ยงที่มีอยู่ใน slide แต่ไม่มี owner, trigger และ response budget จะไม่ช่วยเมื่อเกิดเหตุจริง. PM ต้องทำให้ risk ถูกพูดถึงเป็นจังหวะปกติของ project ไม่ใช่เฉพาะตอนมี crisis.
...
- **Output:** Risk Register, response plan and Issue Log with Change Request IDs where needed.
- **Authority:** PM within delegated threshold; Steering Committee for high/critical or excess-reserve decisions.
- **Next use:** L14 uses risk allocation and accepted changes for vendor control.
